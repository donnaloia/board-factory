...
# Handoff prose summarized in ``docs/board-exporter.md``.
_DEFAULT_GAME_ENGINE_INSTRUCTIONS = (
    ...
    "do not infer links only from space layout. interaction_graph land edges list "
    "from_space_ids (all perimeter tiles for a design), trigger_space_design_id, and "
    "target_panel_id — match land on any listed tile or any tile with that design id. "
    "spaces[].assets.animation may be null — "
    "skip AnimationPlayer setup when absent; when present it is an object with "
    "path, encoding, fps, duration_ms, frame_count, loop_strategy, provider, "
    "model_id, animation_prompt, and notes. Perimeter rects use the same math as "
    "app/domains/spaces/geometry.py:resolve_position and app/frontend/views/board_svg.py. "
    "tech-spec/board-layout/prose.md explains the grid; real boards may differ from the default skeleton."
)
    ...
    if isinstance(ig, list):
        for i, e in enumerate(ig):
            ...
            for k in (
                ...
                fids = e.get("from_space_ids")
                if not isinstance(fids, list) or not fids:
                    errs.append(f"polish: interaction_graph[{i}].from_space_ids must be non-empty")
                elif e.get("from_space_id") != fids[0]:
                    errs.append(
                        f"polish: interaction_graph[{i}].from_space_id must equal from_space_ids[0]"
                    )
                act = e.get("trigger", {}).get("actions") if isinstance(e.get("trigger"), dict) else None
                if isinstance(act, list) and act:
                    a0 = act[0]
                    if isinstance(a0, dict) and a0.get("target_panel_id") != e.get("target_panel_id"):
                        errs.append(
                            f"polish: interaction_graph[{i}] action target_panel_id mismatch"
                        )
