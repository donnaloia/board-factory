...
Every board has:
  ...
  * a set of ``cells`` rows (see the ``domains.spaces`` package).

Layout (per ``app/ARCHITECTURE.md``):

  * ``repository.py`` — DB I/O against ``board_games`` + ``owned_boards``,
                        plus the on-disk root resolver.
  * ``mockup_prompt.py`` — OpenAI-Images **setup mockup** prompt string from catalog
                        layout + user text (used by ``routes_api`` generate-mockup).
