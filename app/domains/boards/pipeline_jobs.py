"""Board Factory pipeline job workers (``domains.boards``).

Plain functions bound to the in-process JobRunner.
...
def analyze(job: Job, cancel: threading.Event, *, openai_key: str) -> float:
    ...
    import domains.boards.repository as boards_repo  # noqa: PLC0415
    import domains.spaces.repository as spaces_repo  # noqa: PLC0415

    if result["style_prompt"]:
        boards_repo.set_style_prompt(bid, result["style_prompt"])

    cell_updates: list[tuple[str, str, str]] = []
    for did, prompt in result["designs"].items():
        if prompt:
            cell_updates.append(("perimeter", did, prompt))
    for pid, prompt in result["panels"].items():
        if prompt:
            cell_updates.append(("functional", pid, prompt))
    if result["centerpiece"]:
        cell_updates.append(("centerpiece", "centerpiece", result["centerpiece"]))
    if cell_updates:
        spaces_repo.bulk_update_prompts(bid, cell_updates)
