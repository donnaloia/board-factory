"""HTTP URLs for board assets with consistent cache-busting.

Every ``/asset/…`` link that reflects an on-disk file under the board store
should include ``?t=<mtime_ms>`` where ``mtime_ms`` matches ``FileStat.mtime_ms``
(:func:`storage.board_store.BoardStore.stat`). Use **milliseconds**, not
seconds: truncating mtime caused stale tiles after promote when the deferred
board SVG refresh reused the same ``?t`` within one POSIX second.

For routes that are not tied to one store file (e.g. composed ``frame.png``),
use :func:`wall_clock_ms` so the query param stays numeric ms without
overloading file-mtime semantics.
