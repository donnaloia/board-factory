...
Design choices:
- Hot jobs live in memory; terminal snapshots persist to SQLite so the tray
  survives process restarts. Queued/running jobs are still lost on restart.
  Live + history assets persist on disk as before.
...
class JobRunner:
    ...
    def __init__(self, max_history: int = 50) -> None:
        ...
        self._hydrate_from_db(max_history)

    def _hydrate_from_db(self, max_history: int) -> None:
        try:
            from services import job_runs as jr  # noqa: PLC0415

            restored = jr.load_recent_jobs(max_history)
        except Exception:
            return
        # SQLite returns newest-first; replay history oldest → newest for the deque.
    ...
    async def _run(
        ...
        loop = asyncio.get_running_loop()
        logger.info(
            "job start id=%s op=%s target=%s label=%s",
            job.id,
            job.operation,
            job.target,
            job.label,
        ...
        try:
            ...
            elapsed = job.elapsed()
            logger.info(
                "job end id=%s op=%s status=%s elapsed_s=%.2f err=%s",
                job.id,
                job.operation,
                job.status,
                elapsed,
