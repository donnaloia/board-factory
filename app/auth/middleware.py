...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        user_id = auth_services.resolve_session_cookie(cookie_value) if cookie_value else None
        user = auth_services.find_by_id(user_id) if user_id else None
        ...
        if user is None:
            ...
            target = "/register" if auth_services.is_setup_required() else "/login"
...
def current_user(request: Request) -> auth_services.User | None:
    """Dependency: returns the user attached by AuthMiddleware (or None)."""
    return getattr(request.state, "user", None)


def require_user(request: Request) -> auth_services.User:
