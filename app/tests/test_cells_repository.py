...
def test_seeded_board_creates_one_cell_per_design_and_panel(isolated_repo, seeded_board, board_id):
    ...
    spaces = [c for c in cells if c.kind == "space"]
    assert len(spaces) == 19  # default catalog seeds 19 board space designs.
    panels = [c for c in cells if c.kind == "panel"]
...
def test_find_id_resolves_category_to_kind(isolated_repo, seeded_board, board_id):
    ...
    assert cells_repo.find_id(board_id, "panels", "panel_left_top") is not None
    assert cells_repo.find_id(board_id, "centerpiece", "centerpiece") is not None
    assert cells_repo.find_id(board_id, "spaces", "does-not-exist") is None
    assert cells_repo.find_id(board_id, "garbage", "x") is None
...
def test_update_space_kind_validates_value(isolated_repo, seeded_board, board_id):
    ...
    with pytest.raises(ValueError):
        cells_repo.update_space_kind(board_id, "corner_tl", "garbage")
    assert cells_repo.update_space_kind(board_id, "missing-slug", "event") is False
...
def test_save_catalog_preserves_cell_ids_for_unchanged_cells(
    ...
    after = {(c.kind, c.slug): c.id for c in cells_repo.list_for_board(board_id)}
