...
Design choices:
- Hot jobs live in memory; terminal snapshots persist to the database so the tray
...
class JobRunner:
    ...
    def _hydrate_from_db(self, max_history: int) -> None:
        try:
            ...
            restored = jobs_repo.load_recent_jobs(max_history)
        except Exception:
            return
        # ``load_recent_jobs`` returns newest-first; replay oldest → newest for the deque.
    ...
    async def _run(
        ...
        try:
            ...
            try:
                ...
                jobs_repo.persist_terminal(job)
            except Exception:
                pass
            self._publish(job)
