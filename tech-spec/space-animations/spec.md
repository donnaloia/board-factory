...
- **Pipeline sandbox:** isolated package under **`pipeline/`**, parallel to `pipeline/boardfactory/` and `pipeline/cardfactory/` — see **§9**. Stakeholders may say **`pipeline/space-animations`**; the importable Python package directory **SHOULD use underscores** (e.g. **`pipeline/space_animations/`**) so ``domains.spaces.animations.pipeline_jobs`` can `import space_animations` the same way board workers import `boardfactory`. If a hyphenated folder is required for non-Python assets only, document an explicit import path strategy.
...
## 8. Provider and generation strategy (**MVP decision: OpenAI hybrid**)

| Topic | Decision (MVP) |
| --- | --- |
| **Primary API** | **OpenAI `gpt-image-2` (`/v1/images/edits`)** + **local PIL morph**. One edit call per candidate produces a "destination" frame (e.g. "subtle glow pulse"); PIL `Image.blend` interpolates source→destination→source for the in-between frames. See `pipeline/space_animations/providers/openai.py`. |
| **First-frame anchoring** | Trivially anchored — frame 0 IS the source PNG; the morph triangle wave returns to the source at the loop boundary. |
| **Loop closure** | Default `crossfade`; the morph already produces a visually seamless boundary so closure is mostly insurance. |
| **Output encoding** | GIF default, APNG opt-in via `SPACE_ANIMATIONS_ENCODING=apng`. |
| **Cost / latency** | 3 OpenAI edit calls per request (~3 × $0.011 ≈ $0.033 at `low` quality). 30–60 s wall-clock typical. The per-user OpenAI / ChatGPT key (Account → Connections) is required; missing key surfaces in the side panel. |
| **Offline / tests** | `SPACE_ANIMATIONS_PROVIDER=mock` selects the synthetic provider used by pytest. |
| **Future** | When a true OpenAI image-to-video / Sora endpoint lands, register a sibling provider in `factory.select_provider`; the morph becomes a fallback. |
