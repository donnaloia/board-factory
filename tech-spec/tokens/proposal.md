...
**Related:** [Backend pipeline — request to art](../../docs/diagrams/pipeline.md), [Card Factory spec](../card-factory/spec.md) (isolated pipeline + gated steps). **Frontend:** reuse board templates, side panel, and job UX — see **§8**.
...
|--------|----------------------|---------------------|----------------------------|
| **Package isolation** | `pipeline/boardfactory/` | `pipeline/cardfactory/` **must not** import `boardfactory` | New tree **`pipeline/tokenfactory/`** — **no** `from boardfactory import …` inside it. |
...
| **App boundary** | `domains.boards/pipeline_jobs.py` + `enqueue_pipeline_job` | `domains.cards/pipeline_jobs.py` | `domains.tokens/pipeline_jobs.py` + Token Factory routes — pipeline stays HTTP-ignorant. **Templates/JS** reuse board shell, side panel, jobs — **§8**. |
...
flowchart LR
  ...
  subgraph Pipe["pipeline/tokenfactory/"]
    L[Design lock]
    K[Key poses]
...
| **5** | **Palette + cleanup** | Reuse Board Factory–style **quantize_to_palette**, **de-speckle** / single-pixel flicker removal. | Could duplicate small helpers inside `tokenfactory` to avoid import (Card Factory precedent). |
...
**Out of scope for the pipeline package** (stays in `app/`): all of the above is **FastAPI routes + templates + static JS**; `pipeline/tokenfactory/` remains unaware of HTML.
...
- **No** HTTP handlers inside `pipeline/tokenfactory/`.
...
- [x] Package name: **`tokenfactory`** (`pipeline/tokenfactory/`; renamed from `board_tokens`).
...
Treat **board tokens** like **Card Factory**: **isolated pipeline package**, **gated design lock**, **ordered steps table**, **``pipeline_jobs`` workers + SSE**, **workspace + manifest**. Reuse **Board Factory’s mental model** for **atomic generate → cleanup → history → promote** and **orchestrate loops**, but **do not** fold token logic into `draw_cell` — keep a dedicated **`generate_token_still`** path so cell and token lifecycles stay separable and export stays a thin **stage** over both trees.

When this proposal is accepted, next artifacts are: **JSON Schema for `manifest.json`**, **catalog `tokens[]` shape**, and **`domains.tokens.pipeline_jobs`** workers wired to **no-op** or mock provider in CI.
