...
| 5   | Review               | (HTTP routes)                              | `app/domains/boards/routes_api.py` (actions → runner)                                                 | Browser                                                                                                                      |
...
- Per-board ownership lives in `owned_boards`; nested board routes resolve the board id via `deps.require_nested_board` (URL must match the logged-in user).
