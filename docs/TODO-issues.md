...
- **Codebase sweep:** Python, templates, static JS, routes (`/panels/…` vs future naming), `board_svg` categories, `domains.cells`, `domains.boards`, `jobs/pipeline_adapters`, pipeline `boardfactory`, docs (`project-export-spec`, ARCHITECTURE), and **any API** that exposes `kind` to the client.  
